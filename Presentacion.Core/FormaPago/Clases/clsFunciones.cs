...
namespace Presentacion.Core.FormaPago.Clases
{
    public class clsFunciones
    {
        ...
        // Foro de Microsoft, (Longitud de 40 Caracteres)
        public class CreaTicket
        {
            ...
            {
                var valor = Math.Round(Convert.ToDecimal(total), 2).ToString("N2");

                if (par1.Length < 20)
                {
                    while (par1.Length < 20)
                    {
                        par1 += " ";
                    }
                }
            ...
            public void AgregaTotalesPorcentaje(string par1, decimal total)
            {
                var valor = Math.Round(Convert.ToDecimal(total), 2).ToString("N2");
            ...
            public void AgregaArticulo(string Articulo, decimal precio, decimal cant, decimal subtotal)
            {
                ...
                string LineaCantidades = "";

                if (Articulo.Length <= 40)
                {
                    LineaArticulo = Articulo;
                    LineaCantidades = cant.ToString("N3") + " X " + precio.ToString("N2") + " = " + subtotal.ToString("N2");
                    line.AppendLine(LineaArticulo);
                    TextoDerecha(LineaCantidades);
                }
                else
                {
                    MessageBox.Show("El articulo Pasa los 40 caractener");
                }
            ...
            public void pd_PrintPage(object sender, PrintPageEventArgs ev) // Este Metodo Define las dimensiones de la Hoja para Poder imprimir
            {
