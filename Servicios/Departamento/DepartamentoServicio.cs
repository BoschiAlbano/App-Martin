...
namespace Servicios.Departamento
{
    public class DepartamentoServicio : IDepartamentoServicio
    {
        ...
        {
            Expression<Func<Dominio.Entidades.Departamento, bool>> filtro = x => x.Descripcion.Contains(cadenaBuscar)
        ...
        {
            return _unidadDeTrabajo.DepartamentoRepositorio.Obtener(x => x.ProvinciaId == provinciaId && x.EstaEliminado == false, "Provincia")
                .Select(x => new DepartamentoDto
                {
                    Id = x.Id,
                    Provincia = x.Provincia.Descripcion,
