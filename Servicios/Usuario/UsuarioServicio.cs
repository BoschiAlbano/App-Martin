...
namespace Servicios.Usuario
{
    public class UsuarioServicio : IUsuarioServicio
    {
        ...
        {
            //Expression<Func<Dominio.Entidades.Usuario, bool>> filtro = x => x.Nombre.Contains(cadenaBuscar);

            //if (!mostrarTodos)
            //{
            //    filtro = filtro.And(x => !x.EstaEliminado); Abria que hacer otro metodo (string, "Usuaiors")
            //}
            ...
            var listaUsuario = empleados.Select(emple => new UsuarioDto
            {
                ...
                NombreUsuario = emple.Usuarios.Any()
                    ...
                    : "NO ASIGNADO",
                Password = emple.Usuarios.Any()
                    ? emple.Usuarios.FirstOrDefault()?.Password
                    : null,
                ...
                Eliminado = emple.Usuarios.Any()
                    ...
                    : false,
            }).ToList().Where(x => x.NombreEmpleado.Contains(cadenaBuscar) 
            || x.ApellidoEmpleado.Contains(cadenaBuscar));// no busca Cuando estan en mayusculas
        ...
        public void CambiarPassword(long idEmpleado, string password)
        {
            var _Empleado = _unidadDeTrabajo.UsuarioRepositorio.Obtener(idEmpleado);
            _Empleado.Password = password;

            _unidadDeTrabajo.UsuarioRepositorio.Modificar(_Empleado);
            _unidadDeTrabajo.Commit();
        }
    }
}
